...
def activate_bound_trading():
    threading.Thread(target=bound_trading).start()
...
def bound_trading():
    ...
    for stock, price_bound in PRICE_BOUNDS.items():
        current_price = get_tradePrice(STOCK_URLS[stock])
        if current_price is None:
            print("None")
            results.append((stock, "N/A", price_bound['low'], price_bound['high'], "No action")) #appends list
            continue

        if current_price <= price_bound['low']:
            create_order(stock, qty=1, side='buy')
            print("bought")
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "Bought")) #appends list

        elif current_price >= price_bound['high']:
            create_order(stock, qty=1, side='sell')
            print("sold")
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "Sold")) #appends list
        else:
            print("None")
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "No action")) #appends list

    print("Bound Trading Results:", results) #prints list
...
def bound_trading_screen():
    # Create a new window for the Bound Trading
    bound_window = tk.Toplevel(root)
    bound_window.title('Bound Trading')
    bound_window.geometry('800x600')

    # Create a label for the screen title
    bound_label = tk.Label(bound_window, text="Bound Trading Results", font=('TkDefaultFont', 18, 'bold'))
    bound_label.grid(row=0, column=0, columnspan=4, padx=10, pady=10)

    # Call bound trading and capture the results
    results = bound_trading()
    print(results)


 # Display results in a table format
    for idx, (stock, current_price, low_bound, high_bound, action) in enumerate(results, start=2):
        stock_label = tk.Label(bound_window, text=stock, font=('TkDefaultFont', 12))
        stock_label.grid(row=idx, column=0, padx=10, pady=5)

        price_label = tk.Label(bound_window, text=f"${current_price:.2f}" if current_price != "N/A" else "N/A", font=('TkDefaultFont', 12))
        price_label.grid(row=idx, column=1, padx=10, pady=5)

        low_bound_label = tk.Label(bound_window, text=f"${low_bound}", font=('TkDefaultFont', 12))
        low_bound_label.grid(row=idx, column=2, padx=10, pady=5)

        high_bound_label = tk.Label(bound_window, text=f"${high_bound}", font=('TkDefaultFont', 12))
        high_bound_label.grid(row=idx, column=3, padx=10, pady=5)

        action_label = tk.Label(bound_window, text=action, font=('TkDefaultFont', 12, 'bold'))
