    ...
    def login_screen():
        def validate_login():
            ...
            if username == fake_username and password == fake_password:
                if API_KEY and SECRET_KEY:  # Ensure API keys are provided
                    HEADERS = {
                    ...
                    if not validate_api_keys():
                        messagebox.showerror("API Key Error", "Invalid API Key or Secret Key. Please try again.")
                        return  # Do not proceed to the application
                    else:
                        login_window.destroy()
                        orderingGUI()  # Proceed to the main application
                else:
                    error_label.config(text="Please provide API keys!", fg="red")
            else:
        ...
        tk.Label(login_window, text="API Key:").pack(pady=5)
        api_key_entry = tk.Entry(login_window, width=30)
        api_key_entry.pack(pady=5)

        tk.Label(login_window, text="Secret Key:").pack(pady=5)
        secret_key_entry = tk.Entry(login_window, width=30, show="*")
