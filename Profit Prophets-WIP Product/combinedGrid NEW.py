...
def bound_trading():
    ...
    for stock, price_bound in PRICE_BOUNDS.items():
        ...
        if current_price is None:
            results.append((stock, "N/A", price_bound['low'], price_bound['high'], "No action"))
            continue

        if current_price <= price_bound['low']:
            create_order(stock, qty=1, side='buy')
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "Bought"))
        elif current_price >= price_bound['high']:
            create_order(stock, qty=1, side='sell')
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "Sold"))
        else:
            results.append((stock, current_price, price_bound['low'], price_bound['high'], "No action"))

    print("Bound Trading Results:", results)
